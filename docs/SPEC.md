...
- **Horizontal axis = time**, user's local timezone, epoch-ms geometry, at a
  ...
  ~43px/hour floor below which horizontal scrolling returns. Hour ticks
  (half-hour on dense days), labeled both edges of the scroll range. Window
  covers all games on the selected day, extended past midnight for overnight
  finishers. A "now" line renders only on days with live games and is an
  isolated element (its clock does not re-render the grid).
